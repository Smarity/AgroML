...
class transformerCNN:
    ...
    def _transformerEncoder(
        ...
        # Feed Forward Part
        x = layers.Conv1D(filters=ff_dim, kernel_size=(n_kernel,), padding="same", activation=activation)(res)
        x = layers.Dropout(dropout)(x)
    ...
    def buildModel(
        ...
        x = self._transformerEncoder(inputs, headSize, nHeads, ffDim, nKernel, dropout=attentionDropout)
        for _ in range(nBlocks-1):
            x = self._transformerEncoder(x, headSize, nHeads, ffDim, nKernel, dropout=attentionDropout)
    ...
    def bayesianOptimization(
        ...
        Arguments:
            ...
            ffDimList: (list of ints) -> [minval, maxval]
            ...
            nKernelList: (list of ints) -> [minval, maxval]
                Size of kernel in CNN
            ...
            activationList: (list of str) ->['activationFunction1', 'activationFunction2',...]
        ...
        dim_ffDim = Integer(low=ffDimList[0], high=ffDimList[1], name='ffDim')
        ...
        dim_nKernel = Integer(low=nKernelList[0], high=nKernelList[1], name='nKernel')
        ...
        self.dimensions = [
            dim_headSize, dim_nHeads, dim_ffDim, dim_nBlocks, dim_nHiddenLayers, dim_nHiddenNeurons,
            dim_nKernel, dim_attentionDropout, dim_mlpDropout, dim_mlpAactivation, dim_epochs]
        
        # default parameters
        self.defaultParameters = [
            headSizeList[0], nHeadsList[0], ffDimList[0], nBlocksList[0], nHiddenLayersList[0], 
            nHiddenNeuronsList[0], nKernelList[0], attentionDropoutList[0], mlpDropoutList[0],
            mlpActivationList[0], epochsList[0]]
        ...
        model = self.buildModel(
            ...
            ffDim = bayesianBestParameters.x[2], 
            nBlocks = bayesianBestParameters.x[3], 
            nHiddenLayers = bayesianBestParameters.x[4], 
            nHiddenNeurons = bayesianBestParameters.x[5], 
            nKernel = bayesianBestParameters.x[6], 
            attentionDropout = bayesianBestParameters.x[7], 
            mlpDropout = bayesianBestParameters.x[8],
            mlpActivation = bayesianBestParameters.x[9])
