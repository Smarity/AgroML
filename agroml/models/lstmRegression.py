...
class LongShortTermMemory:
    ...
    def buildModel(
        ...
        '''
        Arguments:
            nLstmLayers {int} - Number of layers
            lstmUnits {int} - Number of units in lstm layer
        ...
        x = layers.LSTM(units=lstmUnits, activation = 'tanh', return_sequences=True, use_bias=True, unroll=False, 
                        recurrent_dropout = 0, recurrent_activation = 'sigmoid')(inputs)
        for i in range(nLstmLayers-1):
            x = layers.LSTM(units=lstmUnits, activation = 'tanh', return_sequences=True, use_bias=True, unroll=False, 
                            recurrent_dropout = 0, recurrent_activation = 'sigmoid')(x)
        x = layers.Flatten()(x)
    ...
    def bayesianOptimization(
        ...
        dim_nLstmLayers = Integer(low=nLstmLayersList[0], high=nLstmLayersList[1], name='nLstmLayers')
        ...
        self.dimensions = [
            dim_nLstmLayers, dim_lstmUnits, dim_hiddenLayers, dim_neurons, dim_activation, dim_optimizer, dim_epochs
        ]
        
        # default parameters
        self.defaultParameters = [
            nLstmLayersList[0], lstmUnitsList[0], hiddenLayersList[0], neuronsList[0], activationList[0], 
            optimizerList[0], epochsList[0]
        ]
        ...
        def fitnessFunction(**params):
            ...
            try:
                ...
                model = self.buildModel(
                    nLstmLayers = params['nLstmLayers'], 
        ...
        # best model
        model = self.buildModel(
            nLstmLayers = bayesianBestParameters.x[0], 
            lstmUnits = bayesianBestParameters.x[1], 
            hiddenLayers = bayesianBestParameters.x[2], 
            neurons = bayesianBestParameters.x[3],
            activation = bayesianBestParameters.x[4],
            optimizer = bayesianBestParameters.x[5])
        ...
        bestParams = {
            ...
            'activation': bayesianBestParameters.x[4],
            'optimizer': bayesianBestParameters.x[5],
            'epochs': bayesianBestParameters.x[6],
        }
