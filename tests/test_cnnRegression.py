...
def test_saveAndLoadModel():
    ...
    mlModel.saveModel(
        model = trainedModel,
        fileName = 'tests/test-models/cnnModelTest')
    
    assert os.path.exists('tests/test-models/cnnModelTest.h5'), 'The model was not saved'

    loadedModel = mlModel.loadModel(fileName = 'tests/test-models/cnnModelTest.h5')
...
def test_checkPredictionDimensionsMultipleTimeStep():
    ...
    # Daset split by year
    xTrain, xTest, yTrain, yTest = splitDataByPercentageWithLagDays(
        df, 
