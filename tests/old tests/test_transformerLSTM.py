...
def test_dataInputTypes():
    ...
    mlpModel = transformerLSTM(xTrain, xTest, yTrain, yTest)
...
def test_saveAndLoadModel():
    ...
    mlModel.saveModel(
        model = trainedModel,
        fileName = 'tests/test-models/transformerLstmModelTest')
    
    assert os.path.exists('tests/test-models/transformerLstmModelTest.h5'), 'The model was not saved'

    loadedModel = mlModel.loadModel(fileName = 'tests/test-models/transformerLstmModelTest.h5')
